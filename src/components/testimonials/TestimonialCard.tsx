...
function getQuoteTypography(quote: string) {
  const length = quote.trim().length

  if (length > 360) return "text-xs sm:text-[13px] leading-relaxed"
  if (length > 310) return "text-[13px] sm:text-sm leading-relaxed"
  if (length > 260) return "text-sm sm:text-[15px] leading-relaxed"
  return "text-sm sm:text-base leading-relaxed"
}

export default function TestimonialCard({ name, payout, quote, imageSrc }: TestimonialCardProps) {
  const quoteTypography = getQuoteTypography(quote)

  return (
    <div className="px-2 sm:px-3 py-2 h-full">
      <div className="group relative h-full">
        <Card className="relative flex h-full min-h-[420px] max-h-[420px] sm:min-h-[480px] sm:max-h-[480px] bg-zinc-900/70 backdrop-blur-lg border border-amber-500/20 rounded-3xl overflow-hidden transition-all duration-500 group-hover:border-amber-400/60 group-hover:shadow-2xl group-hover:shadow-amber-500/30 group-hover:bg-zinc-900/80 group-hover:scale-[1.02]">
          ...
          <CardContent className="relative flex h-full flex-col items-center text-center gap-4 sm:gap-5 p-5 sm:p-7">
            ...
            <div className="relative flex-1 w-full px-1 flex items-center">
              <p className={`text-zinc-100/90 italic font-light tracking-wide text-pretty break-words ${quoteTypography}`}>
                "{quote}"
              </p>
            </div>
            ...
            <div className="mt-auto space-y-2">
              <div className="text-white font-bold text-base sm:text-lg tracking-wide">{name}</div>
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-amber-500/20 via-amber-600/20 to-orange-500/20 border border-amber-500/40 rounded-full backdrop-blur-sm">
                ...
                <span className="text-amber-400 font-bold text-sm sm:text-base">{currency(payout)}</span>
              </div>
            </div>
          </CardContent>
